...
def load_data(stt, prompt_path, audio_list_path):
    with open(prompt_path, "r") as file:
        transcriptions = file.readlines()
    with open(audio_list_path, "r") as file:
        audio_filenames = [f"{filename.strip()}.m4a" for filename in file.readlines()]
    assert len(transcriptions) == len(audio_filenames), "Mismatch between number of transcriptions and audio filenames."
...
def process_audios(stt, df, total_audios, path, logger):
    ...
    for idx, row in tqdm(df.iterrows(), total=total_audios, desc="Processing audios"):
    # for idx, row in tqdm(df.head(5).iterrows(), total=5, desc="Processing audios"):
